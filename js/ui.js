...
import { inverseApplyOffset, canvasToMap, mapToCanvas, applyOffset } from './math.js';
import { sampleCurve, pathLengthM, buildDense } from './bezier.js';
import { draw, resetView, zoomStep, animZ, clearMeasure } from './renderer.js';
...
function closeLoop() {
    ...
    function onSpd(v) { document.getElementById('spdV').textContent = parseFloat(v).toFixed(2); if (state.ws && state.ws.readyState === WebSocket.OPEN) sw({ cmd: 'set_speed', value: parseFloat(v) }); }
    function onSmp(v) { state.sampleStepM = parseFloat(v); document.getElementById('smpV').textContent = (state.sampleStepM * 100).toFixed(0) + 'cm'; }
    function onCornerRound(v) { state.cornerRound = v; state.anchors.forEach((a, i) => { if (!a.hIn && !a.hOut && i > 0 && i < state.anchors.length - 1) a.round = v; }); updAnchorList(); draw(); }
    function onCornerRadius(v) { state.cornerRadiusM = parseFloat(v); document.getElementById('cradV').textContent = state.cornerRadiusM.toFixed(2); draw(); }
    function toggleRound(i) { const a = state.anchors[i]; if (a && !a.hIn && !a.hOut) { a.round = !a.round; updAnchorList(); draw(); } }

    function sendPose(x, y, yaw) {
      if (sw({ type: 'pose_estimate', x, y, yaw })) {
        log(`Pose: (${x.toFixed(2)},${y.toFixed(2)}) ${(yaw * 180 / Math.PI).toFixed(0)}°`, 'ok');
      }
      // Update state.robot langsung tanpa tunggu AMCL
...
function updStatus(s) {
    ...
    function updScan(p) { state.scan = p || []; state.scanT = Date.now(); draw(); }
...
function onRot(v) { state.mapRot = parseFloat(v); document.getElementById('rotV').textContent = Math.round(state.mapRot) + '°'; draw(); }
    function rotStep(d) { state.mapRot = (state.mapRot + d + 360) % 360; document.getElementById('rot').value = state.mapRot; document.getElementById('rotV').textContent = Math.round(state.mapRot) + '°'; draw(); }
    function rotReset() { state.mapRot = 0; document.getElementById('rot').value = 0; document.getElementById('rotV').textContent = '0°'; draw(); }

function undoLast() { if (state.anchors.length) { state.anchors.pop(); updAnchorList(); draw(); } }
    function clearAll() { state.anchors = []; updAnchorList(); draw(); }
    function delAnchor(i) { state.anchors.splice(i, 1); updAnchorList(); draw(); }
    ...
    function penUp() { state.penDrag = null; }
    function penDelete(e) { const ai = hitAnchor(e.offsetX, e.offsetY); if (ai >= 0) state.anchors.splice(ai, 1); else state.anchors.pop(); updAnchorList(); draw(); }
...
function setTool(t) {
      ...
      document.getElementById('hint').textContent = {
        ...
        pose: 'Klik tahan + drag = set posisi & arah state.robot',
    ...
    function resetView() {
      if (state.mapImg) { const cw = document.getElementById('cw'); const sx = cw.clientWidth / state.mapImg.naturalWidth; const sy = cw.clientHeight / state.mapImg.naturalHeight; state.vs = Math.min(sx, sy) * .9; state.ts = state.vs; state.vx = (cw.clientWidth - state.mapImg.naturalWidth * state.vs) / 2; state.vy = (cw.clientHeight - state.mapImg.naturalHeight * state.vs) / 2; }
      else { state.vx = 0; state.vy = 0; state.vs = 1; state.ts = 1; } draw();
    }
    function zoomStep(d) { const f = d > 0 ? 1.25 : .8; state.ts = Math.max(.05, Math.min(50, state.ts * f)); state.za = { x: state.cvEl.width / 2, y: state.cvEl.height / 2 }; if (!state.zraf) animZ(); }

    state.cvEl.addEventListener('wheel', e => {
      e.preventDefault();
      const f = Math.exp(-(Math.max(-60, Math.min(60, e.deltaY)) / 60) * .12);
      state.za = { x: e.offsetX, y: e.offsetY }; state.ts = Math.max(.05, Math.min(50, state.ts * f)); if (!state.zraf) animZ();
    }, { passive: false });
    function animZ() {
      const d = state.ts - state.vs; if (Math.abs(d) < .0008) { state.vs = state.ts; draw(); state.zraf = null; return; }
      const st = d * .28, ns = state.vs + st, f = ns / state.vs;
      state.vx = state.za.x - (state.za.x - state.vx) * f; state.vy = state.za.y - (state.za.y - state.vy) * f; state.vs = ns; draw(); state.zraf = requestAnimationFrame(animZ);
    }

function log(msg, type = 'in') {
      const b = document.getElementById('lb');
      const cls = type === 'ok' ? 'ok' : type === 'er' ? 'er' : 'in';
      const state.ts = new Date().toLocaleTimeString('id', { hour12: false });
      b.innerHTML += `<div class="${cls}"><span class="lts">[${state.ts}]</span> ${msg}</div>`;
      b.scrollTop = b.scrollHeight; while (b.children.length > 60) b.removeChild(b.firstChild);
    }
    function setWS(c) { document.getElementById('wsDot').className = 'state.ws-dot' + (c ? ' on' : ''); document.getElementById('wsLabel').textContent = c ? 'Terhubung' : 'Tidak terhubung'; }


export { tc, closeLoop, sendPath, cmd, onSpd, onSmp, onCornerRound, onCornerRadius, toggleRound, sendPose, updStatus, updPose, updScan, loadMap, loadYaml, onRot, rotStep, rotReset, undoLast, clearAll, delAnchor, updAnchorList, hitAnchor, hitHandle, penDown, penMove, penUp, penDelete, setTool, resetView, zoomStep, animZ, log, setWS };

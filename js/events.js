...
// ── Canvas events ─────────────────────────────────────────────────────
state.cvEl.addEventListener('click', e => {
  if (state.suppressNextClick) { state.suppressNextClick = false; return; }
  if (state.tool === 'measure' && !state.pan) { state.measPts.push(canvasToMap(e.offsetX, e.offsetY)); draw(); }
});

state.cvEl.addEventListener('contextmenu', e => {
  e.preventDefault();
  if (state.tool === 'measure') { if (state.measPts.length) { state.measPts.pop(); draw(); } }
  else if (state.tool === 'pen') { penDelete(e); }
  else undoLast();
});

state.cvEl.addEventListener('mousedown', e => {
  if (e.button !== 0) return;
  if (state.tool === 'pan') { state.pan = true; state.ps = { x: e.offsetX - state.vx, y: e.offsetY - state.vy }; state.cvEl.style.cursor = 'grabbing'; }
...
state.cvEl.addEventListener('mouseup', e => {
  if (state.pan) { state.pan = false; if (state.tool === 'pan') state.cvEl.style.cursor = 'grab'; }
...
state.cvEl.addEventListener('wheel', e => {
  ...
  state.za = { x: e.offsetX, y: e.offsetY }; state.ts = Math.max(.05, Math.min(50, state.ts * f));
  if (!state.zraf) animZ();
}, { passive: false });
...
document.addEventListener('keydown', e => {
  ...
  if (e.key === ' ') { setTool(state.tool === 'pan' ? 'pen' : 'pan'); e.preventDefault(); }
});

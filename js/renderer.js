...
import { niceGridStep, mapToImgPx, mapToCanvas, canvasToMap, pxPerMetre } from './math.js';
...
export function resizeCv() {
  ...
  if (W > 0 && H > 0) { state.cvEl.width = W; state.cvEl.height = H; }
  draw();
}
...
export function showFrd(t) { const e = document.getElementById('frd'); e.textContent = t; e.style.display = 'block'; }
export function hideFrd() { document.getElementById('frd').style.display = 'none'; }

// ── Measurement ───────────────────────────────────────────────────────
function drawMeasurement() {
  const allPts = state.measHover ? [...state.measPts, state.measHover] : state.measPts;
...
export function clearMeasure() { state.measPts = []; state.measHover = null; draw(); }

export function animZ() {
